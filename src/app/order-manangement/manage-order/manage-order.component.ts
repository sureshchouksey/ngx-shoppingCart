...
export class ManageOrderComponent implements OnInit {
  ...
  deliveryStatusByUser = new FormControl('');
  loading: boolean;
  ...
  ngOnInit() {
     this.searchForm = this.formBuilder.group({
        ...
        deliveryStatusByUser:this.deliveryStatusByUser
    })
     this.LIMITS = [
      { key: 10, value: 10 },
      { key: 20, value: 20 },
    ...
    search(){
    let searchUser= new SearchOrder();
    if(this.searchForm.value.username)
      searchUser.userName =this.searchForm.value.username;
    if(this.searchForm.value.area)
      searchUser.area =this.searchForm.value.area;
    if(this.searchForm.value.agentName)
     searchUser.agentName =this.searchForm.value.agentName;
    if(this.searchForm.value.milkType)
      searchUser.milkType =this.searchForm.value.milkType;
    if(this.searchForm.value.deliveryStatusByUser)
     searchUser.deliveryStatusByUser =this.searchForm.value.deliveryStatusByUser;

    console.log(searchUser);
    this.userService.searchOrder(searchUser)
    	.subscribe(agents => {
